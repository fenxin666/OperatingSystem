...
#define FIX_Q 14           /* Number of fractional bits. (17.14 Format) */
...
static inline fixed_point_t fix_frac(int n, int d) {
  return __mk_fix((int64_t)n * FIX_F / d);
}

/* Returns X rounded to the nearest integer. */
static inline int fix_round(fixed_point_t x) {
  if (x.f >= 0) return (x.f + FIX_F / 2) / FIX_F;
  else return (x.f - FIX_F / 2) / FIX_F;
}

/* Returns X truncated down to the nearest integer. */
static inline int fix_trunc(fixed_point_t x) {
  return x.f / FIX_F;
}
...
/* Returns X + N (integer). */
static inline fixed_point_t fix_add_int(fixed_point_t x, int n) {
  return __mk_fix(x.f + n * FIX_F);
}

/* Returns X - N (integer). */
static inline fixed_point_t fix_sub_int(fixed_point_t x, int n) {
  return __mk_fix(x.f - n * FIX_F);
}

/* Returns X * Y. */
static inline fixed_point_t fix_mul(fixed_point_t x, fixed_point_t y) {
  return __mk_fix((int64_t)x.f * y.f / FIX_F);
}

/* Returns X * N (integer). */
static inline fixed_point_t fix_mul_int(fixed_point_t x, int n) {
  return __mk_fix(x.f * n);
}

/* Returns X / Y. */
static inline fixed_point_t fix_div(fixed_point_t x, fixed_point_t y) {
  return __mk_fix((int64_t)x.f * FIX_F / y.f);
}

/* Returns X / N (integer). */
static inline fixed_point_t fix_div_int(fixed_point_t x, int n) {
  return __mk_fix(x.f / n);
}

/* [修复] 补上丢失的 fix_scale 函数 */
